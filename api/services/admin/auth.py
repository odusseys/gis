...
def create_user(first_name: str, last_name: str, email: str):
    with session_scope() as session:
        ...
        invitation_token = jwt.encode(
            payload, ADMIN_JWT_SECRET, algorithm='HS256').decode('utf-8')
        if should_send_mail:
            send_mail([email], "Your Playground Admin invite",
                      f"Follow the following link:\n\nhttps://admin.playground-app.io/signup?invitation_token={invitation_token}")
...
def login_user(email: str, password: str):
    with session_scope() as session:
        res = session.query(AdminUser, AdminAccount).filter(
            AdminUser.email == email,
            AdminUser.id == AdminAccount.user_id
        ).first()
        if res is None:
            raise Unauthorized(error_code="USER_NOT_FOUND")
        user, account = res
...
def list_users():
    with session_scope() as session:
        res = session.query(AdminUser, AdminAccount).outerjoin(
            AdminAccount).all()
        return [dict(
            id=user.id,
